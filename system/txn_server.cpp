...
// ==========
RC
TxnManager::process_remote_request(const SundialRequest* request,
    SundialResponse* response)
{
    ...
    // XXX(zhihan): using latch to make sure prepare, commit, abort cannot be
    ...
    switch(request->request_type()) {
        ...
        case SundialRequest::TERMINATE_REQ:
            ...
            if (_txn_state == RUNNING) {
                ...
                pthread_mutex_unlock(&_latch);
                return ABORT;
            } else if (_txn_state == PREPARED) {
                // after prepared before commit
                rc = termination_protocol();
                // continue the rest of the commit protocol, in case commit
                // msg is never sent
                break;
            }
