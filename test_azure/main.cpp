...
/*    try {
        ...
        azure::storage::cloud_block_blob blob3 = container.get_block_blob_reference(
                U("my-directory/my-sub-directory/my-blob-3"));
...
*/
    ...
    try {
        ...
        azure::storage::blob_request_options options4;
        azure::storage::operation_context context4;
        pplx::task<void> upload_task = blob4.upload_text_async(U("test"), condition4, options4, context4);
    }
    catch (const std::exception &e) {
        std::wcout << U("Error: ") << e.what() << std::endl;
    }
